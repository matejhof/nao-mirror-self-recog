...
class Central:
    ...
    def central_execute(self, NN):
        ...
        # self.videoRecorderProxy.startVideoRecord("/Home", "myvideo")
        ...
        while not rospy.is_shutdown():
            try:
                ...
                if(float(self.x>0) and float(self.y>0)):
                    ...
                    self.set_head_angles(joint_angle_prediction[0, 0], joint_angle_prediction[0, 1])
                    self.Shoulder_pitch_L_moveto(joint_angle_prediction[0, 2])
                    self.Shoulder_roll_L_moveto(joint_angle_prediction[0, 3])
                    self.Elbow_roll_L_moveto(joint_angle_prediction[0, 4])
                    rospy.sleep(5.0)
                    # Go back to home position
                    self.set_head_angles(0, 0)
                    self.Shoulder_pitch_L_moveto(1.402034044265747)
                    self.Shoulder_roll_L_moveto(0.13341593742370605)
                    self.Elbow_roll_L_moveto(-0.9817180633544922)
                    rospy.sleep(5.0)
...
if __name__ == '__main__':
    ...
    n_in, n_h, n_out = 2, 10, 5

    NN = nn.Sequential(nn.Linear(n_in, n_h),
                     nn.Sigmoid(),
                     nn.Linear(n_h, n_out),
                     nn.Sigmoid())

    NN.load_state_dict(torch.load(pathname + '/Trained_Networks/touching'))
    NN.eval()
